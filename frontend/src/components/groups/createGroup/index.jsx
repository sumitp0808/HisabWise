...
import { createGroupService } from "../../../api/groupApi";
...
export default function CreateGroup() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      window.location = configData.VIEW_GROUP_URL + createResponse.data.Id;
    },
  });
  ...
  useEffect(() => {
    const getEmails = async () => {
      ...
      const filtered = list.filter((email) => email !== currentUser);
  ...
  const toggleMember = (email) => {
    const exists =
      values.groupMembers.includes(email);
